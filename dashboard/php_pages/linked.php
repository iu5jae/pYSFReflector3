...
<table class="table">
  <thead class="thead-dark">
    <tr>
      ...
      <th scope="col">DG_ID_(Description)</th>
      <th scope="col">Gateway_IP</th>
      <th scope="col">Refl_outgoing_port</th>
      <th scope="col">0/60</th>
      <th scope="col">Connected_from</th>
      <th scope="col">Muted</th>
      <th scope="col">Locked</th>
      <th scope="col">Status</th>
      <th scope="col">Back_to_home</th>
      <th scope="col">BTH_in_secs</th>
      <th scope="col">Activated_secs</th>
      <th scope="col">Missing_secs</th>
    </tr>
  </thead>
  <tbody>
 <?php
...
while ($row = $res->fetchArray()) {
    ...
    echo "<th scope='row'>{$nr}</th><td>{$row['call']}</td><td>{$row['DGID']}</td><td>{$row['IP']}</td><td>UDP: {$row['port']}</td><td>{$row['TC']}</td><td>{$row['CF']}</td><td>{$row['LO']}</td><td>{$row['LK']}</td><td>{$row['T_HOLD']}</td><td>{$row['BTH_DGID']}</td><td>{$row['BTH_TOUT']}</td><td>{$row['BTH_TCORR']}</td><td>{$diff}</td>\n";
	echo "</tr>";
	++$nr;
}
?>
